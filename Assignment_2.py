...
curDir="C:\\Users\\AlexD\\Desktop\\Currently working on\\python\\datafiles" #Directory where only the datafiles are stored
...
for filenam in a2df['filename']: # For each file
    ...
    dataframe_collection[fname]=dataframe_collection[fname].where((dataframe_collection[fname]['Length'] > 2000) & (dataframe_collection[fname][fname]>0)) #If Length is less than 2000 or the third column is less than zero make Length nan
...
df.to_csv("MET586_Python_Assignment_2.tsv",header=True,index=True,sep='\t') #print the final dataframe to a tsv file. 
